...
class MIGraph(Module):
    ...
    def __init__(
        ...
        self.lat_lon_grid = torch.stack(torch.meshgrid(self.latitudes, self.longitudes, indexing="ij"), dim=-1)
...
if __name__ == '__main__':
    ...
    # 初始化模型（使用较小分辨率便于测试，可根据需要调整）
    net = MIGraph(
        params=None,
        input_res=(120, 240),  # 或 (720, 1440) 用于全分辨率测试
        input_dim_grid_nodes=101,
        output_dim_grid_nodes=93,
        hidden_dim=256,        # 可降低到128节省显存
        processor_layers=4,    # 可降低到2快速测试
        mesh_level=5,
        multimesh=True
    ).to(device)
    
    # 构造输入: (batch, input_features, lat, lon)
    input = torch.randn(1, 101, 120, 240).to(device)
