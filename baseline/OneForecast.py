...
class MeshGraphMLP(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.recompute_activation:
            return self.custom_silu_linear_forward(x)   # B. 显存优化模式
        return self.default_forward(x)   # A. 标准模式
...
def local_refine_sphere_triangles(
    ...
    centroids = get_face_centroids(vertices, faces)  # (F,3)  每个三角形面片的质心（Centroid）
    ...
    faces_local = faces[ mask_local.numpy() ]   # 待细化区域
    faces_other = faces[~mask_local.numpy() ]   # 保持原样区域
    ...
    refined = merge_two_meshes(mesh_local, mesh_other)  # 网格合并
    refined = remove_duplicate_vertices(refined)        # 清理重复点
...
class Graph:
    ...
    def __init__(
        ...
        self.refine_specs = refine_specs        # [(0.0, 30.0, 105.0, 160.0, 1), (10.0, 30.0, -95.0, -35.0, 1)]
        self.complex_edge = complex_edge        
        self.lon_lat_complex_edge = lon_lat_complex_edge    # [(23.5, -88.7, 25.1, -80.2), (37.0, -75.0, 33.0, -73.0)]

        # flatten lat/lon gird
        self.lat_lon_grid_flat = lat_lon_grid.permute(2, 0, 1).view(2, -1).permute(1, 0)

        # 从正二十面体开始，不断切分三角形（Mesh Level），生成覆盖全球的均匀三角形网格。这避免了极点附近的采样过度问题。
        ...
        # 局部加密 (do_local_refine)：这是一个非常实用的工业逻辑。
        # 它允许你在特定区域（如某个国家或海域）进行更高密度的网格切分，实现“局部高精度预报”。
        if self.do_local_refine:
            ...
            for (lat_min, lat_max, lon_min, lon_max, extra_splits) in self.refine_specs:
                mesh_local = local_refine_sphere_triangles(
                    ...
                    complex_edge=complex_edge,  # False
                    latlon_pairs=lon_lat_complex_edge   # [(23.5, -88.7, 25.1, -80.2), (37.0, -75.0, 33.0, -73.0)]
                )
            refined_finest = mesh_local


        # 将不同精度的网格（从粗到细）合并在一起。
        # 这样做能让模型在 Processor 阶段既能通过粗网格获取“全球视野”，又能通过细网格捕捉“局部细节”。
        if multimesh:
            if self.do_local_refine:
    ...
    def create_mesh_graph(self, verbose: bool = True) -> Tensor:
        ...
        # 调用 xyz2latlon 和 add_edge_features，将 3D 空间位置（XYZ）和经纬度信息注入到节点和边中
    ...
    # 这是一个异质二分图，连接“原始物理网格点（Grid）”和“计算网格点（Mesh）”
    def create_g2m_graph(self, verbose: bool = True) -> Tensor:
        ...
        # 使用 NearestNeighbors (KNN) 算法。它为每一个地理网格点寻找最近的 4 个 Mesh 节点。
        n_nbrs = 4
        ...
        for i in range(len(cartesian_grid)):
            for j in range(n_nbrs):
                if distances[i][j] <= 0.6 * max_edge_len:  # 只有距离小于 0.6 * max_edge_len 的点才会建立连接。这确保了信息聚合是具有“局部相关性”的
                    src.append(i)
    ...
    def create_m2g_graph(self, verbose: bool = True) -> Tensor:
        ...
        # 它首先计算 Mesh 每个三角形面的中心点（Centroids），然后为每一个地理网格点寻找最近的面中心
        # 由于每个面有 3 个顶点，所以每个 Grid 点会连接到其所在（或最近）面的 3 个 Mesh 顶点。这是一种基于重心的线性插值思想，确保预测结果平滑地还原到地理空间。
        n_nbrs = 1
...
class MeshEdgeBlockMultiHeadGated(nn.Module):
    ...
    def forward(
        ...
        cat_raw = torch.cat([efeat, src_feat_edge, dst_feat_edge], dim=-1)  # [98296, 384]
        gating_all = self.gate_net(cat_raw)  # shape=[#edges, 3*num_heads]  # [98296, 12]

        gating_all = gating_all.view(-1, self.num_heads, 3)                 # [98296, 4, 3]
   
        gating = gating_all.mean(dim=1) # [98296, 3]
...
class MeshNodeBlockMultiHeadAttn(nn.Module):
    ...
    def forward(
        ...
        attn_logits = self.attn_net(efeat)      #  这里只用 edge feature 计算注意力 [98296, 128] -- [98296, 4] 

        with graph.local_scope():  # 在这个作用域内对 graph 的修改是临时的，退出后自动恢复,往 graph.edata 里塞的 "logits"、"score"
            graph.edata["logits"] = attn_logits
            graph.edata["score"] = dgl.nn.functional.edge_softmax(graph, attn_logits)  # 对每个 目标节点 的入边分别做 softmax
            alpha = graph.edata["score"]  

        efeat_heads = efeat.unsqueeze(1) 
        efeat_heads = efeat_heads.expand(-1, self.num_heads, -1)    # [98296, 4, 128]
        weighted_efeat_heads = efeat_heads * alpha.unsqueeze(-1)    # 加权

        with graph.local_scope():
            graph.edata["x"] = weighted_efeat_heads     # 98296, 4, 128
            def reduce_func(nodes):
               
                return {"h_dest": nodes.mailbox["m"].sum(dim=1)}  # 对所有入边求和  这个和论文实现略有不同
            graph.update_all(
        ...
        dst_all = dst_all.view(dst_all.shape[0], -1)    # [12337, 4, 128]--[12337, 512]  “多头”  横向拼接在一起，变成一个长向量
...
class OneForecast(Module):
    ...
    def encoder_forward(self,grid_nfeat: Tensor,) -> Tensor:
    ...
    def custom_forward(self, grid_nfeat: Tensor) -> Tensor:
       ...
       # Encoder-Processor-Decoder（编码器-处理器-解码器）的架构设计，
       # 并且通过多级 Checkpoint（计算检查点）来极大地压缩显存占用。

        (
            mesh_efeat_processed,  # 计算网格的边特征
            mesh_nfeat_processed,  # 计算网格的节点特征
            grid_nfeat_encoded,    # 原始网格编码后的特征（类似于残差连接，留给 Decoder 使用）
        ) = self.encoder_checkpoint_fn(
            self.encoder_forward,  # 将原始物理场特征投影到隐空间  通过 Grid-to-Mesh 操作，将分布在规则地理网格上的信息聚合到抽象的“计算网格（Mesh）”上
            grid_nfeat,
            use_reentrant=False,
            preserve_rng_state=False,
        )
    ...
    def forward( self,grid_nfeat: Tensor,) -> Tensor:
        ...
        # 把你给的 [B, C, H, W] 规则网格输入，变成“ grid node feature 形式”，并在需要时处理分布式切分。   [Num_Nodes, Features]
        invar = self.prepare_input(grid_nfeat, self.expect_partitioned_input, self.global_features_on_rank_0)


        # model_checkpoint_fn (Gradient Checkpointing) 这是一个关键的显存优化技术。
        # 由于气象预报模型通常参数巨大且中间激活值（Activation）很多，直接训练会导致显存溢出（OOM）。
        # 逻辑：它在前向传播时不保存 custom_forward 内部的中间激活值，而在反向传播时重新计算它们。这用额外的计算时间换取了巨大的显存空间节省。

        outvar = self.model_checkpoint_fn(
            self.custom_forward,    # 这是模型真正的“大脑”。它包含了你在代码中看到的 Encoder -> Processor -> Decoder 逻辑。
            invar,
            use_reentrant=False,    # 采用了 PyTorch 推荐的新版 Checkpoint 机制，对 DDP（分布式数据并行）更友好
            preserve_rng_state=False,
        )

        # 将 [Num_Nodes, Features] 重新 view 并 permute 回 [Batch, Channels, Height, Width]
    ...
    def prepare_input(self, invar: Tensor,expect_partitioned_input: bool, global_features_on_rank_0: bool,) -> Tensor:
        ...
        if not self.is_distributed:
            ...
            # invar[0] : [69, 120, 240]  [69, 120×240] = [69, 28800]   [28800, 69]
    ...
    def prepare_output(self,outvar: Tensor,produce_aggregated_output: bool,produce_aggregated_output_on_all_ranks: bool = True,) -> Tensor:
